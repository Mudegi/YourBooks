...
export default function StockMovementsPage() {
  ...
  const loadMovements = async () => {
    try {
      const res = await fetch(`/api/${orgSlug}/inventory/movements`);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const json = await res.json();
        throw new Error(json.error || 'Failed to create movement');
      }
      setForm({ ...form, quantity: '1', unitCost: '0', notes: '', referenceId: '' });
      await loadMovements();
    } catch (err) {
  ...
  if (loading) return <div className="p-6">Loading stock movements...</div>;
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Record purchases, sales, transfers, and returns.</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
        <h2 className="text-lg font-semibold mb-3">New Movement</h2>
          ...
          <div>
            ...
            <div className="flex space-x-2">
              <input
                ...
                onChange={onChange}
                className="mt-1 w-1/2 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
              />
              <input
                name="referenceId"
                placeholder="ID"
          ...
          <tbody className="divide-y divide-gray-200">
            {movements.map((m) => (
              <tr key={m.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-sm text-gray-900">{m.productName} ({m.sku})</td>
